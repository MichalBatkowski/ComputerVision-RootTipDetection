...
class OT2Env(gym.Env):
    def __init__(self, render=False, max_steps=1000, use_wandb=True):
        ...
        self.success_count = 0  # Count successful episodes
    ...
    def step(self, action):
        ...
        # Calculate the reward (negative distance to the goal, scaled)
        distance_to_goal = np.linalg.norm(relative_position)
        reward = float(-distance_to_goal / 10)  # Scale the reward

        # Check termination condition
        terminated = bool(distance_to_goal < 0.01)  # Relaxed threshold for success
        if terminated:
        ...
        if self.use_wandb:
            wandb.log({
                ...
                "success_count": self.success_count,  # Log successful goals
            })
